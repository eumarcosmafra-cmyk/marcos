...
function getDiagnosis(cat: CategoryNode): string {
  if (cat.status === "well_positioned") {
    return `Produto consolidado na posicao ${cat.position.toFixed(1)} para "${cat.topQuery}". Prioridade baixa de correcao -- foco em manter e proteger.`;
  }
    ...
    >
      ...
      <div className="mb-3 flex items-center justify-between">
        ...
        <span className="text-[10px] font-medium" style={{ color: "var(--text-muted)" }}>
          {categories.length} {categories.length === 1 ? "produto" : "produtos"}
        </span>
      </div>
    ...
    <div className="glass-card overflow-hidden">
      ...
      >
        ...
        <div className="relative flex-1 min-w-[200px] max-w-xs">
          ...
          <input
            type="text"
            placeholder="Buscar produto ou query..."
            value={search}
...
export default function ProductMapPage() {
  ...
  async function loadData(useMock: boolean) {
    setLoading(true);
    try {
      const res = await fetch("/api/product-map", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          useMock,
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-sm mt-0.5" style={{ color: "var(--text-tertiary)" }}>
              Mapa estratégico de produtos com treemap visual, diagnóstico automático e priorização
            </p>
          </div>
        </div>
      ...
      {loading && (
        <div className="glass-card flex items-center justify-center gap-3 py-16">
          ...
          <span className="text-sm" style={{ color: "var(--text-muted)" }}>
            Analisando produtos e posições via GSC...
          </span>
        </div>
      )}
      ...
      {!loading && !loaded && (
        <div className="glass-card flex flex-col items-center gap-3 py-20 text-center">
          ...
          <p className="text-sm" style={{ color: "var(--text-muted)" }}>
            Insira um sitemap de produtos ou selecione um site e clique em &quot;Analisar&quot; para gerar o mapa.
          </p>
      ...
      {!loading && loaded && (
        <>
          <div className="grid grid-cols-6 gap-3">
            <KpiCard icon={LayoutGrid} label="Total Produtos" value={kpis.total} accent="text-brand-400" />
